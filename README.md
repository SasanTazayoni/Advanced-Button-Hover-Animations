...
- **Ripple Appearance:** The ripple’s colour is set by the `background` property in `.ripple-button > span`. Its size is controlled by the `width` and `height` properties on `.ripple-button > span` (set to `300px`). The expansion is animated via `transform: scale()` in `@keyframes ripple`. A blur effect can be added with the `filter` property in `.ripple-button > span`, such as `filter: blur(4px)`.
...
- **Hover Transition:** The hover background colour is set in `.ripple-button:hover` and the text colour switches for contrast. The smoothness of this change is controlled by `transition: all 0.6s` in `.ripple-button`.
...
- **Hover Interaction:** The hover effect activates when the user places their cursor over the button. A bright overlay is generated, which gradually fades in over the button and starts to blink in and out. This effect gives the impression that the button is pulsating or "blinking" with each hover.
...
- **Button Background Colour:** As the blinking animation occurs, the button's background colour transitions to a deeper shade, which helps highlight the button's interactivity and further emphasises the blinking effect. The background colour change provides additional visual feedback to the user.
- **Mouse Leave Behaviour:** When the cursor leaves the button, the blinking effect halts, and the overlay fades out. The button's background colour transitions back to its initial shade, ensuring a smooth and seamless return to its resting state.
...
- **Button Background Colour:** As the circles complete their fade-in, the button’s background colour shifts to its active shade by adding the `active` class. This is timed with a `setTimeout` keyed to the last circle’s fade-in delay (`circles.length * 30ms`).
...
- **Button Background Colour:** The resting background colour is set via `.radiate-button { background-color }` and the active colour via `.radiate-button.active { background-color }`. Update both in the CSS to match a specific theme.

- **Circle Colour & Transition Style:** Each circle uses a semi-transparent `background-color` via `rgba()`. The RGB values can be changed to adjust the colour hue, while the alpha is tied to the `opacity` field in `circlesData`. The fade-in behaviour is governed by `.circle { transition: opacity 0.6s ease; }`, which can be adjusted for different easing curves or durations.
...
- **Hover Animation & Grid Logic:** When the cursor enters the button area, a grid is calculated based on the button’s dimensions and a fixed square size (5px). All unfilled grid positions are collected into an array and shuffled once using Fisher-Yates, then popped one by one — giving O(1) placement with no retry loops regardless of how full the grid is. Each square is absolutely positioned and styled with a configurable gradient, giving the effect of individually coloured tiles emerging behind the button’s content.
- **Pixel Rendering:** Each frame of the animation places up to four squares by popping from the pre-shuffled position list. A `Map` keyed by grid position stores direct element references, tracking which cells are filled and enabling O(1) element lookup during removal — no DOM queries needed. This continues until the entire button is pixelated, creating a satisfying gradual fill that feels both energetic and controlled.
...
- **Hover Interaction & Circle Movement:** When the user hovers over the button, four circles rotate in along arc paths using CSS `transform: rotate()`. Each circle shares a common `transform-origin` set to the button’s centre point, so rotating from `±90deg` to `0deg` sweeps each circle inward along a quarter-circle arc. Two circles begin above the button’s visible area and two begin below, each sweeping to a distinct resting position distributed across the button. Their opacity transitions from 0 to 1 during the arc, adding to the effect’s dynamic feel.
- **Button Background Colour:** As the hover effect completes and the circles settle into position, the button’s background colour transitions while the text and border colours invert. This smooth transition suggests that the circles are dissolving to transform the button’s background, providing satisfying visual feedback.
- **Mouse Leave Behaviour:** When the cursor leaves the button, the circles fade out and return to their rotated starting positions, reversing the arc. The background colour reverts to its initial state.
...
The Converge button offers a visually engaging hover effect, where circles appear to dissolve into the button's background, creating a captivating interactive experience. The combination of smooth transitions, fading circles, and background colour change makes this button a striking addition to any interface, perfect for drawing attention and providing intuitive feedback to users.
...
- **Line Colour & Gradient:** The gradient applied to the lines can be customised to match the overall design of a website or application. Change the linear gradient or apply solid colours to the lines for a different look.
...
- **Radial Gradient Background Transition:** Upon hovering, a vibrant radial gradient gradually fades in over 0.6s (`transition: opacity 0.6s ease`). On hover-out, the gradient fades out faster at 0.3s. This asymmetric transition enhances the flow of energy through the button, adding depth and dimension to the interaction.
...
- **Opacity & Colour:** The opacity of each line is calculated dynamically using `Math.random() * maxOpacity`, where `maxOpacity` starts at 0.9 and gradually decreases. This starting value and its decrement rate (`opacityDecreaseRate`, `0.09`) are configurable to control the intensity and fading speed. Line colour is defined in `.line-upwards` and `.line-downwards` and can be replaced with any colour to suit project themes. The radial gradient applied on hover is defined in the `::before` pseudo-element and can also be modified for different colour schemes or effects.
...
- **Colour Styling:** The tile colour is set by `background-color` in the `.square` CSS rule, while the base button style (background, border, and text colour) is managed in `.cascade-button` and its inner `<span>`. These can be easily customised for various visual themes without touching the TypeScript.
...
- **Rising Water Fill:** A semi-transparent overlay, implemented using a `::after` pseudo-element, acts as the rising "water." When the user hovers, this overlay expands upward from the base of the button, creating a smooth fill effect that visually simulates the button being submerged. The height transition is purposefully slowed on hover (0.9s), giving the water time to rise, and sped up on exit (0.3s) to suggest quick drainage.
- **Droplet Animation:** A base pool of droplets is created at initialisation using a `DocumentFragment` and kept in the DOM permanently. On `mouseenter`, the pool grows automatically if the button area requires more droplets than currently available — eliminating per-interaction DOM creation while adapting to any button size. On each spawn tick, a droplet is acquired from the pool, repositioned, and set in motion. Droplets vary in size and fall speed, contributing to a natural, irregular flow. Each droplet rotates slightly and features rounded edges to reinforce the look of a water bead. Droplets are positioned relative to the button using `offsetWidth` and `offsetHeight`, read fresh on each spawn to reflect the current button dimensions. Each droplet stays fully visible for 75% of its fall, then fades out linearly over the remaining 25%, reaching full transparency exactly as it arrives at the bottom. A `setTimeout` returns each droplet to the pool once its transitions complete.
...
- **Water Fill Colour & Speed:** The rising fill colour can be changed to match a brand or theme by modifying `background-color` in `.water-button::after`. The transition speed of the fill effect can be adjusted using the `transition: transform` property in both `.water-button::after` (on hover) and `.water-button:not(:hover)::after` (on exit), depending on whether a serene soak or a sudden splash is to be established.
- **Droplet Frequency & Size:** The spawn interval is fixed at 3ms, but the number of droplets spawned per tick scales automatically with button area — larger buttons spawn proportionally more droplets to maintain consistent coverage. The baseline area (`7000px²`, matching the default button size) controls this ratio and can be adjusted to tune overall density. The droplet size range is controlled via `Math.random() * 5 + 10` — changing this affects whether droplets appear small like mist or large like heavy raindrops.
...
- **Fade Timing:** Each droplet begins fading at 75% through its fall, becoming fully transparent exactly as it reaches the bottom — so no droplet visibly touches the edge. The fade duration is derived from `fallDuration` (currently 25%), so faster-falling droplets also fade more quickly. Adjusting the fade delay multiplier (0.75) controls how early the fade begins; adjusting the fade duration multiplier (0.25) controls how gradually it disappears.
...
- **Background Flicker:** Simultaneously, the button’s background colour transitions from its resting state to its active colour, reinforcing the sense of a sudden jolt or interference.
...
- **Code Rain Generation:** When hovered, the button continuously spawns 1–3 falling trails of Japanese katakana characters (like ｱ, ﾂ, ﾅ, etc.) every 200ms, which animate downward in vertical streams. Each trail begins at a random x-position chosen from a dynamically calculated column grid — character size scales so that exactly `TRAIL_LENGTH` characters span the full button height, and column spacing is derived proportionally from that size to fill the button width. A `ResizeObserver` keeps both values accurate if the button is resized. Available columns are selected using a Fisher-Yates shuffle, ensuring no two simultaneous trails share the same column without relying on an unbounded retry loop. Each trail drops character by character with fading opacity to mimic the signature layered "digital rain" effect. The frequent, small spawns create an organic trickle rather than uniform bursts.
...
- **Trail Length & Speed:** The number of characters in each trail (`TRAIL_LENGTH`) and the delay between them (`delayBetweenChars`, ranging from 50–350ms) determine the animation's pacing and reach. Note that `TRAIL_LENGTH` also drives character sizing — it sets how many characters span the full button height, so reducing it produces larger, bolder glyphs while increasing it gives smaller, denser rainfall. Character size is clamped to a minimum of 8px, so on very short buttons (below roughly `TRAIL_LENGTH × 8px` in height) characters may not span the full button. Adjust these values to create longer, slower trails or short, rapid-fire bursts. The wide speed range ensures noticeable variation between individual trails.
...
- **Trail Spacing:** Column width and count are automatically derived from the character size, which itself scales to the button height via `TRAIL_LENGTH`. Adjusting `TRAIL_LENGTH` is the primary lever — fewer characters per trail means larger glyphs and wider, more spaced columns; more characters produces tighter, denser clustering.
...
- **Glow Colour & Intensity:** The edge glow colour is defined in the `linear-gradient` backgrounds of the `.pulse-top`, `.pulse-bottom`, `.pulse-left`, and `.pulse-right` classes. The gradient transitions from the glow colour to `transparent` and can be adjusted to any colour. The glow's maximum opacity is set by `.pulse-edge.active { opacity: 1; }` — reduce this for a subtler effect.
...
- **Border Style & Colour:** The border colour, width, and alpha of `.vortex-square` can all be changed — a thinner border with lower alpha creates a more ghost-like structure, while a thicker, fully opaque border makes the geometry feel heavier and more mechanical.
